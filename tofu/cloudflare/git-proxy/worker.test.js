...
describe("git-proxy worker", () => {
  ...
  describe("OCI registry proxy (oci.jacobcolvin.com)", () => {
    it("returns 404 for non-/v2/ paths", async () => {
      const request = createRequest("https://oci.jacobcolvin.com/");
      const response = await worker.fetch(request);
    ...
    it("returns 404 for non-v2 paths", async () => {
      const request = createRequest("https://oci.jacobcolvin.com/v1/foo");
      const response = await worker.fetch(request);

      expect(response.status).toBe(404);
    });

    it("proxies /v2/ base endpoint to ghcr.io", async () => {
      const request = createRequest("https://oci.jacobcolvin.com/v2/");
      const response = await worker.fetch(request);

      // ghcr.io returns 401 for unauthenticated /v2/ requests
      expect(response.status).toBe(401);
      expect(response.headers.get("www-authenticate")).toBeTruthy();
    });

    it("proxies manifest requests with path rewriting", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/kclipper/manifests/latest"
      );
      const response = await worker.fetch(request);

      // ghcr.io returns 401, requiring auth
      expect(response.status).toBe(401);
      const wwwAuth = response.headers.get("www-authenticate");
      expect(wwwAuth).toBeTruthy();
      // Verify the www-authenticate header was rewritten to use our token endpoint
      expect(wwwAuth).toContain("oci.jacobcolvin.com/v2/token");
      // Verify the scope was rewritten to remove the org prefix
      expect(wwwAuth).toContain("repository:kclipper:");
      expect(wwwAuth).not.toContain("repository:macropower/kclipper:");
    });

    it("proxies blob requests", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/kclipper/blobs/sha256:abc123"
      );
      const response = await worker.fetch(request);

      // Will return 401 or 404 depending on ghcr.io's response
      expect([401, 404]).toContain(response.status);
    });

    it("proxies tags list requests", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/kclipper/tags/list"
      );
      const response = await worker.fetch(request);

      // ghcr.io returns 401 for unauthenticated requests
      expect(response.status).toBe(401);
    });

    it("handles nested image paths (e.g., charts/mychart)", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/charts/kclipper/manifests/latest"
      );
      const response = await worker.fetch(request);

      expect(response.status).toBe(401);
      const wwwAuth = response.headers.get("www-authenticate");
      // Should rewrite scope for nested paths too
      expect(wwwAuth).toContain("repository:charts/kclipper:");
      expect(wwwAuth).not.toContain("repository:macropower/charts/kclipper:");
    });

    it("rewrites token endpoint scope to add org prefix", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/token?service=ghcr.io&scope=repository:kclipper:pull"
      );
      const response = await worker.fetch(request);

      // Token endpoint should return a valid response (200) or error from ghcr.io
      // The key is that it doesn't 404 - it proxies to ghcr.io
      expect(response.status).not.toBe(404);
    });

    it("handles token requests for nested repos", async () => {
      const request = createRequest(
        "https://oci.jacobcolvin.com/v2/token?service=ghcr.io&scope=repository:charts/kclipper:pull"
      );
      const response = await worker.fetch(request);
