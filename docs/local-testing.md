...
- The devshell tools: `talosctl`, `kubectl`, and `kcl` (kclipper). Enter it with
...
All state (kubeconfig, talosconfig, the Talos cluster state dir) is written under
the git-ignored `.cache/` directory, so the test cluster never touches your real
...
1. **Cluster (`tald:up`).** `talosctl cluster create docker`, pinned to the same
   Talos image and Kubernetes version a live cluster runs (read from
   `clusters/<TALD_TALOS_REF>/talconfig.yaml`, default `main`) rather than
   talosctl's drifting built-in defaults, with `resources/patch-cni-none.yaml`
   (sets `cni: none`, `proxy.disabled: true`, mirrors the live clusters'
   control-plane feature gates and `hostDNS`, relaxes PodSecurity to `privileged`,
   allows scheduling on the control plane, and adds a `kubelet.extraMounts` rbind
   for `/var/openebs/local` so the kubelet can mount the OpenEBS hostpath volumes
   `apps/kube/openebs/local` provisions).
   The create blocks on node-Ready -- which never happens without a CNI -- so the
   `cluster-create` task detaches it (`setsid`); the `wait-apiserver` and `cilium`
   tasks then run, bring the datapath up, and the detached create observes Ready
   and exits on its own. The result is a pristine `cni=none`, kube-proxy-less node.
...
is needed). The Talos
machine-config patch stays YAML
(`.taskfiles/talos-docker/resources/patch-cni-none.yaml`), since `talosctl`
consumes it, not Kubernetes.
...
- **`task tald:up` fails a precondition.** `up` requires `talosctl`, `kcl`,
  `kubectl`, and `yq` on `PATH` -- enter the devshell (`nix develop`/`direnv`). A
  missing or unreachable rootful Docker daemon is not caught by a precondition; it
  surfaces as the detached `talosctl cluster create` failing into
  `.cache/cluster-create.log` and `tald:up` timing out waiting for the talosconfig
  (the `wait-apiserver` step tails that log on timeout). Inspect
  `.cache/cluster-create.log` for the Docker error.
